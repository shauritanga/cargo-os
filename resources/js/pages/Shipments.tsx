...
export default function Shipments() {
  ...
  const filtered = useMemo(() => {
    ...
    return shipments
      ...
      .sort((a, b) => {
        let va: number | string = a[sortField as keyof Shipment] as string;
        let vb: number | string = b[sortField as keyof Shipment] as string;
        if (sortField === 'eta' || sortField === 'created') { va = (a[sortField as keyof Shipment] as Date).getTime(); vb = (b[sortField as keyof Shipment] as Date).getTime(); }
        if (sortField === 'weight') { va = Number(a.weight); vb = Number(b.weight); }
        if (va < vb) return -sortDir; if (va > vb) return sortDir; return 0;
      });
  }, [shipments, statusFilter, typeFilter, modeFilter, search, sortField, sortDir]);
    ...
    <div className="content">
      ...
      <div className="card" style={{ overflow: 'visible' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', flexWrap: 'wrap' }}>
          <div className="search-wrap" style={{ flex: 1, maxWidth: 300, minWidth: 180 }}>
            ...
            <input type="text" placeholder="Search ID, customer, origin, destination…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}/>
          </div>
          <div className="filter-tabs">
            {(['all','transit','delivered','pending','delayed','customs'] as const).map(s => (
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            <select className="sh-select" value={modeFilter} onChange={e => { setModeFilter(e.target.value); setPage(1); }}>
              ...
              {(['Sea','Air','Road','Rail'] as TransportMode[]).map(m => <option key={m}>{m}</option>)}
            </select>
            ...
            <select className="sh-select" value={sortField} onChange={e => { setSortField(e.target.value); setSortDir(1); }}>
            ...
            <div className="col-toggle-wrap" ref={colMenuRef}>
              <button className="btn" onClick={() => setShowColMenu(v => !v)}>
                ...
                <div className="col-toggle-menu">
                  {columns.map((c, i) => (
                    <div key={c.key} className="col-toggle-item"
                      onClick={() => setColumns(prev => prev.map((col, j) => j === i ? { ...col, visible: !col.visible } : col))}>
                      <div className={`col-check${c.visible ? ' checked' : ''}`}>
                        <svg viewBox="0 0 9 7" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M1 3.5l2.5 2.5 5-5"/></svg>
                      </div>
                      {c.label}
                    </div>
                  ))}
        ...
        {/* BULK BAR */}
        {selectedIds.size > 0 && (
          <div className="bulk-bar visible" style={{ borderTop: '1px solid var(--blue-border)', borderRadius: 0 }}>
            <span className="bulk-count">{selectedIds.size} selected</span>
            <button className="btn" style={{ padding:'5px 12px', fontSize: 12 }} onClick={() => bulkUpdateStatus('transit')}>Mark In Transit</button>
            <button className="btn" style={{ padding:'5px 12px', fontSize: 12 }} onClick={() => bulkUpdateStatus('delivered')}>Mark Delivered</button>
            <button className="btn" style={{ padding:'5px 12px', fontSize: 12, color: 'var(--red)', borderColor: 'var(--red-dim)' }} onClick={bulkDelete}>
              <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"><path d="M1 3h12M4 3V2a1 1 0 011-1h4a1 1 0 011 1v1M5 6v5M9 6v5M2 3l1 9a1 1 0 001 1h6a1 1 0 001-1l1-9"/></svg>Delete
            </button>
            <button className="btn" style={{ padding:'5px 12px', fontSize: 12, marginLeft: 'auto' }} onClick={() => setSelectedIds(new Set())}>Clear</button>
          </div>
        )}
      </div>
          ...
          <table className="sh-table">
            <thead>
              <tr>
                ...
                {visibleCols.map(c => (
                  <th key={c.key} className={sortField === c.key ? 'sorted' : ''} onClick={() => handleSort(c.key)}>
                    {c.label}
                    <span className="sort-icon">{sortField === c.key ? (sortDir > 0 ? '↑' : '↓') : '↕'}</span>
                  </th>
                ))}
                <th style={{ width: 40 }}/>
              </tr>
            ...
            <tbody>
              {pageItems.length === 0 ? (
                <tr><td colSpan={visibleCols.length + 2} style={{ padding: 0 }}>
                  <div className="empty-state">
                    <svg viewBox="0 0 40 40" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"><rect x="4" y="8" width="32" height="28" rx="3"/><path d="M13 4v8M27 4v8M4 18h32"/></svg>
                    <p>No shipments match your filters</p>
                  </div>
                </td></tr>
              ) : pageItems.map(s => {
                const isIntl = s.type === 'international';
                const isSelected = selectedIds.has(s.id) || detailId === s.id;
                return (
                  <tr key={s.id} className={isSelected ? 'selected' : ''} onClick={() => setDetailId(s.id)}>
                    <td style={{ paddingLeft: 16 }}>
                      <input type="checkbox" className="sh-checkbox" checked={selectedIds.has(s.id)}
                    ...
                    {visibleCols.map(c => {
                      ...
                      if (c.key === 'weight') return <td key="weight">{s.weight.toLocaleString()} kg</td>;
                      if (c.key === 'mode') return <td key="mode"><ModeIcon mode={s.mode}/>{s.mode}</td>;
                      if (c.key === 'cargoType') return <td key="cargo">{s.cargoType || '—'}</td>;
                      if (c.key === 'containers') return <td key="containers">{isIntl ? (s.containers || '—') : '—'}</td>;
                      if (c.key === 'eta') return <td key="eta">{fmtDate(s.eta)}</td>;
                      if (c.key === 'created') return <td key="created">{fmtDate(s.created)}</td>;
                      if (c.key === 'status') return <td key="status"><Badge variant={s.status}>{STATUS_LABEL[s.status]}</Badge></td>;
                      return <td key={c.key}/>;
                    })}
                    <td>
                      <button style={{ background:'none',border:'none',cursor:'pointer',color:'var(--text-3)',padding:4,borderRadius:5,display:'grid',placeItems:'center' }}
                        onClick={e => { e.stopPropagation(); setDetailId(s.id); }}>
                        <svg viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" style={{ width:13,height:13 }}><path d="M5 2h7v7"/><path d="M12 2L2 12"/></svg>
                      </button>
                    </td>
                  </tr>
                );
          ...
          <div className="dp-header">
            <div>
              ...
              <div style={{ marginTop:4 }}>
                ...
                {' '}
                <span style={{ display:'inline-flex',alignItems:'center',gap:4,fontSize:11,fontWeight:600,padding:'2px 8px',borderRadius:20,background:detailShipment.type==='international'?'var(--blue-dim)':'var(--green-dim)',color:detailShipment.type==='international'?'var(--blue)':'var(--green)' }}>
                  {detailShipment.type==='international'?'🌐 International':'🏠 Domestic'}
                </span>
              </div>
            </div>
          ...
          <div className="dp-scroll">
            ...
            {(() => {
              ...
              return (
                <div style={{ padding:'16px 20px',borderBottom:'1px solid var(--border)' }}>
                  <div style={{ display:'flex',justifyContent:'space-between',fontSize:11.5,color:'var(--text-3)',marginBottom:8 }}>
                    <span>Progress</span>
                    <span style={{ fontWeight:600,color:'var(--text-2)' }}>{doneCount}/{steps.length} steps</span>
                  </div>
                  <div className="progress-steps">
                    {steps.map((st, i) => (
                      <div key={i} className={`ps-step${st.done?' done':st.active?' active':''}`}/>
                    ))}
                  </div>
                </div>
              );
            ...
            <div className="dp-section">
              ...
              <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:12 }}>
                <div style={{ flex:1,background:'var(--bg-3)',borderRadius:7,padding:'8px 12px',textAlign:'center' }}>
                  <div style={{ fontSize:10,color:'var(--text-3)',textTransform:'uppercase',letterSpacing:'.05em' }}>{detailShipment.originCountry}</div>
                  <div style={{ fontSize:14,fontWeight:600,marginTop:2 }}>{detailShipment.origin}</div>
                </div>
                ...
                <div style={{ flex:1,background:'var(--bg-3)',borderRadius:7,padding:'8px 12px',textAlign:'center' }}>
                  <div style={{ fontSize:10,color:'var(--text-3)',textTransform:'uppercase',letterSpacing:'.05em' }}>{detailShipment.destCountry}</div>
                  <div style={{ fontSize:14,fontWeight:600,marginTop:2 }}>{detailShipment.dest}</div>
                </div>
              </div>
              ...
              {getTimeline(detailShipment).map((st, i) => (
                <div key={i} className="tl-item">
                  ...
                  <div className="tl-body">
                    <div className="tl-label" style={st.warn?{color:'var(--amber)'}:{}}>{st.label}</div>
                    <div className="tl-meta">{st.done?(i===0?fmtDate(detailShipment.created):'Completed'):st.active?'In progress':'Awaiting'}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="dp-section">
              ...
              <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:8 }}>
                ...
                <button className="btn" style={{ justifyContent:'center',color:'var(--red)' }} onClick={() => deleteShipment(detailShipment.id)}>Delete</button>
              </div>
            </div>
          </div>
        </div>
